...
class CrossAttnBlock(nn.Module):
    def __init__(self, d, nheads=4, ff=4, attn_dropout: float = 0.0):
        super().__init__()
        self.sa = nn.MultiheadAttention(d, nheads, batch_first=True, dropout=attn_dropout)
        self.ca = nn.MultiheadAttention(d, nheads, batch_first=True, dropout=attn_dropout)
    ...
    def forward(self, p, d, p_mask=None, d_mask=None):
        ...
        concat_mask = torch.cat([p_mask, d_mask], dim=1)
...
class DrugGeneEncoder(nn.Module):
    def __init__(
        ...
        if use_xattn:
            self.xblk = CrossAttnBlock(d_model, nheads=2, ff=2, attn_dropout=self.attn_dropout)
            if return_sequences:
    ...
    def forward(self, p_vec, batch_data=None, drug_node_seq=None, p_mask=None, d_mask=None):
        ...
        if self.return_sequences:
            ...
            if self.freeze_gnn:
                ...
                node_features, pyg_batch = self.gnn(batch_data)
            d_seq, d_mask = batch_to_sequence(node_features, pyg_batch, self.max_nodes)
            d = self.dproj(self.d_in(d_seq))
        else:
            if batch_data is not None:
                ...
                assert d_vec is not None
            d = self.dproj(self.d_in(d_vec)).unsqueeze(1)
...
class GeneDrugTargetClassifier(nn.Module):
    def __init__(self, dim_p=768, emb_dim=300, d_model=512, gnn_ckpt=None, freeze_gnn=True, 
        ...
        self.encoder = DrugGeneEncoder(dim_p, emb_dim, d_model, gnn_ckpt, freeze_gnn, 
                                       gnn_type, use_xattn, return_sequences, attn_dropout=attn_dropout)
